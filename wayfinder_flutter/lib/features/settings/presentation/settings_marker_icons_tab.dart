      ...
      children: [
              ...
              children: [
                for (var i = 0; i < sorted.length; i++) ...[
                  ...
                  _MarkerIconEntryTile(
                    entry: sorted[i],
                    label: _displayLabel(l10n, sorted[i]),
                    previewColor: previewColor,
                    onUploadSvg: () => _pickAndUploadSvg(sorted[i].key),
                    onEdit: () => _editIcon(sorted[i]),
                    onDelete: () => _deleteIcon(sorted[i]),
                  ),
                ],
              ],
            );
        ...
        Card(
          ...
          child: ExpansionTile(
            ...
            children: [
              for (var i = 0; i < bundledSvgIcons.length; i++) ...[
                if (i > 0) const Divider(height: 1),
                ListTile(
                  leading: MapMarkerIcon(
                    color: previewColor,
                    iconName: bundledSvgIcons[i].key,
                    width: 32,
                    height: 32,
                  ),
                  title: Text(localizedMarkerIconLabel(l10n, bundledSvgIcons[i].key)),
                  subtitle: Text(bundledSvgIcons[i].key),
                  trailing: IconButton(
                    tooltip: l10n.markerIconsUploadSvgAction,
                    icon: const Icon(Icons.upload_file),
                    onPressed: _isBusy
                        ? null
                        : () => _pickAndUploadSvg(bundledSvgIcons[i].key),
                  ),
                ),
              ],
            ],
